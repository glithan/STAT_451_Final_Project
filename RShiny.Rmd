...
```{r}
growth <- ggplot(mtcars, aes(x = wt, y = mpg)) +
  geom_point(color = "blue") +
  labs(title = "Growth Plot", x = "Weight", y = "Miles per Gallon")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(),
    mainPanel(
      plotOutput("myPlot")  # Match the output ID
    )
  )
)
...
server <- function(input, output) {
  output$myPlot <- renderPlot({
    growth  # Directly return the ggplot object
  })
}
